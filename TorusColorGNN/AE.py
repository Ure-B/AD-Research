...
def create_graph(vertices, features, k=16):  # Increased k
...
def train_vae(model, data, optimizer, epochs=100, lambda_smooth=0.1):
    model.train()
    for epoch in range(epochs):
        ...
        optimizer.step()
        if epoch % 10 == 0:
            print(f"Epoch {epoch}, Loss: {loss.item():.6f}, Smoothness: {smoothness_loss.item():.6f}")
...
model = AE(in_channels=3, hidden_dim=64, latent_dim=32)
